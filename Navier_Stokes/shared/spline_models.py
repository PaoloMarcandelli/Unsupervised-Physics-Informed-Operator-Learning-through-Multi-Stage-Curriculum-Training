...
def interpolate_states(hidden, offset, orders_v=[2,2], create_graph=True, retain_graph=True):
    ...
    ψ, v, grad_v, ω, grad_ω, lap_ω = interpolate_2d_velocity(
    ...
    ω      = ω.permute(0, 2, 3, 1)
    v      = v.permute(0, 2, 3, 1)
    grad_ω = grad_ω.permute(0, 2, 3, 1)
    lap_ω  = lap_ω.permute(0, 2, 3, 1)

    return ω, v, grad_ω, lap_ω
...
def interpolate_2d_velocity(weights,offsets,orders=[2,2], create_graph=True,retain_graph=True):
	...
	if offset_key in kernel_buffer_velocity.keys():
		...
		kernels = torch.zeros(1,1+2+4+1+2+1,(orders[0]+1),(orders[1]+1),2,2).to(device)
		...
		#vorticity
		kernels[0:1,7:8,...] = kernels[0:1,5:6] - kernels[0:1,4:5]
		# gradients of vorticity
		kernels[0:1,8:10] = grad(kernels[0:1,7:8],offsets, create_graph,retain_graph)
		
		# laplace of vorticity
		kernels[0:1,10:11] = div(kernels[0:1,8:10],offsets, create_graph,retain_graph)
		
		
		kernels = kernels.reshape(1,1+2+4+1+2+1,(orders[0]+1)*(orders[1]+1),2,2).detach()
		
		# buffer kernels
		kernel_buffer_velocity[offset_key] = kernels
		save_buffers()
	
	#output = F.conv2d(F.pad(weights[...,1:-1,1:-1],pad=(1,1,1,1),mode='circular'),kernels[0]) #N-S Dataset 
	output = F.conv2d(F.pad(weights,pad=(1,1,1,1),mode='circular'),kernels[0]) #N-S Dataset
	#print(output.shape)
	# CODO: to be even more efficient, we could separate interpolation in x/y direction
	return output[:,0:1],output[:,1:3],output[:,3:7],output[:,7:8],output[:,8:10], output[:,10:11]
