...
# ======== Phases ========
phases = [
    {'name': 'full_loss_1', 'record': True,  'epochs': epochs, 'λ_sup': 1.0, 'λ_res':1.0}
]
...
df_err = pd.DataFrame(err[pad:-pad,pad:-pad])
csv_name = f"residual_internal_multistage.csv"  # assegna un nome coerente, es. 'phisfno', 'pino'
df_err.to_csv(csv_name, index=False, header=False)
print(f"Saved residual field in {csv_name}")
